...
- **Unattended from 22:55:** a scratchpad script (`orchestrate.sh`, log `run/transcripts/orchestrate.log`) runs `deploy transcripts all LANG` as each check ends, then `deploy plan-windows run/transcripts-fp100m fp100m 16 haskell:35 lean:25 agda:20 nix:10 bend:10` and the eval corpora below; it stops at the first failure. If it is gone (reboot), the same steps by hand:
