...
The automatic puller publishes the newest complete cloud checkpoint locally at
`run/rtx5070ti-checkpoints/wiki-bpe10m-global.checkpoint`. Transfers are staged
under that directory and renamed atomically, so this path always names a
complete checkpoint even if SSH disconnects during a pull.

`wiki-generate` searches both `run/*.checkpoint` and nested
`run/*-checkpoints/*.checkpoint`, so it automatically selects these pulled
